...
void loop()
{
  ...
  checkScaling(nextionEtxCome && dataBuff[0] == 0x01 && dataBuff[1] == 0x02 && dataBuff[2] == 0x00); // 65 01 02 00 FF FF FF ch1
  testTextbox(nextionEtxCome && dataBuff[0] == 0x06 && dataBuff[1] == 0x05 && dataBuff[2] == 0x00);
  resetNextionEtxCome();
...
void testTextbox(boolean _flag)
{
	if(_flag)
	{
		Serial.println();
...
{
  tick_state = false;
  if (millis() - previousMillis > INTERVAL) //10ms  ___|^^|_____10ms____|^^|___
  {

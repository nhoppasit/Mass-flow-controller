...
int stateValue; // ใช้การเก็บค่าที่ชื่อ stateValue
unsigned long T_LED = 0; //การเก็บค่าตัวเลขที่เป็นจำนวนเต็มที่ชื่อ T_LED
unsigned long Showvalue ; //การเก็บค่าตัวเลขที่เป็นจำนวนเต็มที่ชื่อ Showvalue
boolean BlinkLED1 = false; // ใช้จำสถานะBlinkLED
boolean TimingLED1 = false; //ใช้จำสถานะTimingLED
boolean ADC1 = false; //ใช้จำสถานะADC1

boolean BlinkLED2 = false; // ใช้จำสถานะBlinkLED
boolean TimingLED2 = false; //ใช้จำสถานะTimingLED
boolean ADC2 = false; //ใช้จำสถานะADC2

boolean BlinkLED3 = false; // ใช้จำสถานะBlinkLED
boolean TimingLED3 = false; //ใช้จำสถานะTimingLED
boolean ADC3 = false; //ใช้จำสถานะADC3

boolean BlinkLED4 = false; // ใช้จำสถานะBlinkLED
boolean TimingLED4 = false; //ใช้จำสถานะTimingLED
...
void taskStart1 (boolean _flag)
{
  if (_flag)
  {
    ...
    {
      T_LED = timer + millis(); //โดยทุกๆtimer โปรแกรมจะส่งสัญญาณHIGH ตามเวลาที่กำหนด และสัญญาณLOWเมื่อหมดเวลา 
...
void taskOnOff1 (boolean _flag)
{
  if (_flag)
  {
    if (millis() > T_LED) //ถ้าmills()มีค่ามากกว่าT_LED     
    {      
      T_LED = timer + millis(); //โดยทุกๆtimer โปรแกรมจะส่งสัญญาณHIGH และสัญญาณLOWสลับกัน เกิดเป็นไฟกระพริบที่ขา12 โดยค่าของ millis() จะเพิ่มค่าอัตโนมัติตามการรันโปรแกรม
      Serial.println(T_LED);
      Serial.print("T_LED: ");
      state = !state ;
      if (state == 1)
  ...
  if (_flag)
  {
   ...
   // SP1 = 0;
    ...
    else
    {
      ...
      A = ( 4095 * SPN1 ) / FSN1 ; //แปลง 0-100 ให้อยู่ในช่วง 0-4095 byte      
      voltage1(A); //เซ็ตค่าDAC Voltage1
      Serial.print("A = ");
      Serial.println(A, BIN);
      Serial.println(A);
      
      B = ( FSN1 * A ) / 4095 ; //แปลง 0-4095 ให้อยู่ในช่วง 0-100 sccm
      Serial.print("B(sccm) = ");
      Serial.println(B);      
...
void TimingStart2 (boolean _flag)
{
  if (_flag)
  {
    ...
    else
    {
      ...
      C = ( 4095 * SPN2 ) / FSN2 ; //ฟังชั่นเที่ยบบรรหยัดไตรยางmassflow แปลง 0-100 ให้อยู่ในช่วง 0-4095 byte
      voltage2(C); //Set DAC voltage
      Serial.print("C = ");
      Serial.println(C, BIN);
      Serial.println(C);
      
      D = ( FSN2 * C ) / 4095 ; //แปลง 0-4095 ให้อยู่ในช่วง 0-100 sccm
      Serial.print("D(sccm) = ");
      Serial.println(D);      
...
void TimingStart3 (boolean _flag)
{
  if (_flag)
  {
    ...
    else
    {
      ...
      E = ( 4095 * SPN3 ) / FSN3 ; //แปลง 0-100 ให้อยู่ในช่วง 0-4095 byte
      voltage3(E);//Set DAC voltage
      Serial.print("E = ");
      Serial.println(E, BIN);
      Serial.println(E);
       
      F = ( FSN3 * E ) / 4095 ; //แปลง 0-4095 ให้อยู่ในช่วง 0-100 sccm
      Serial.print("F(sccm) = ");
      Serial.println(F);
...
void TimingStart4 (boolean _flag)
{
  if (_flag)
  {
    ...
    else
    {
      ...
      G = ( 4095 * SPN4 ) / FSN4 ; //แปลง 0-100 ให้อยู่ในช่วง 0-4095 byte
      voltage4(G); //Set DAC voltage
      Serial.print("G = ");
      Serial.println(G, BIN);
      Serial.println(G);
       
      F = ( FSN4 * G ) / 4095 ; //แปลง 0-4095 ให้อยู่ในช่วง 0-100 sccm
      Serial.print("F(sccm) = ");
      Serial.println(F);
...
void TimingStartONOFF1(boolean _flag)
{
  if (_flag)
  {
    ...
    else
    {
      ...
      A = ( 4095 * SPN1 ) / FSN1 ; //ฟังชั่นเที่ยบบรรหยัดไตรยางmassflow  กำหนด FS1 = 100 แปลง 0-100 ให้อยู่ในช่วง 0-4095 byte
      voltage1(A); //Set DAC voltage
      Serial.print("A = ");
      Serial.println(A, BIN);
      Serial.println(A);
       
      B = ( FSN1 * A ) / 4095 ;  //กำหนด FS1 = 100 //แปลง 0-4095 ให้อยู่ในช่วง 0-100 sccm
      Serial.print("B(sccm) = ");
      Serial.println(B);
      Serial.println("_______________________________ END CHANNEL 1 _______________________________");
      Serial.println("End ask.");
...
void checkChannel2(boolean _flag)
{
  if (_flag)
  {
    ...
    else
    {
      ...
      C = ( 4095 * SPN2 ) / FSN2 ; //แปลง 0-100 ให้อยู่ในช่วง 0-4095 byte
      voltage2(C); //Set DAC voltage
      Serial.print("C = ");
      Serial.println(C, BIN);
      Serial.println(C);
...
void taskMeasureFlow1(boolean _flag)
{
  if (_flag)
  {
    /*if (millis() > Showvalue)
    {
      Showvalue = 2000 + millis();
      Serial.print("Showvalue: ");
